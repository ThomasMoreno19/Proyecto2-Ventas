...
describe('validateCliente', () => {
  ...
  it('should return cliente if found', async () => {
    const mockCliente: Cliente = { cuil: '12345678', nombre: 'Cliente 1', apellido: 'Test', telefono: '123456789', email: 'cliente@example.com', deletedAt: null };
    ...
    expect(prisma.cliente.findUnique).toHaveBeenCalledWith({ where: { cuil: '12345678', deletedAt: null } });
  });
  ...
  it('should throw NotFoundException if cliente not found', async () => {
    ...
    await expect(validateCliente(prisma, '99999999')).rejects.toThrow('Cliente con CUIL 99999999 no encontrado');
    expect(prisma.cliente.findUnique).toHaveBeenCalledWith({ where: { cuil: '99999999', deletedAt: null } });
  });
});
